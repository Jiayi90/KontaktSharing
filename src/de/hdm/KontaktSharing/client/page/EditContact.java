...
public class EditContact extends CommonPage {
	...
	private void renderPage() {
		ClientsideSettings.getKontaktSharingAdministration()
				.getAllEigenschaft(new AsyncCallback<Vector<Eigenschaft>>() {
					@Override
					...
					public void onSuccess(final Vector<Eigenschaft> eigenschaften) {
						ClientsideSettings.getKontaktSharingAdministration().getAllEigenschaftauspraegungByKontakt(
								kontakt, new AsyncCallback<Vector<Eigenschaftauspraegung>>() {
										...
										for (final Eigenschaft eigenschaft : eigenschaften) {
											...
											for (int i = 0; i < currentAuspraegung.size(); i++) {
												...
												deleteButton.addClickHandler(new ClickHandler() {
													...
													public void onClick(ClickEvent event) {
														ClientsideSettings.getKontaktSharingAdministration()
																.delete(auspraegung, new ReloadPage());
													}
												});
												...
												editButton.addClickHandler(new ClickHandler() {
													...
													@Override
													public void onClick(ClickEvent event) {
														String newVal = Window.prompt(eigenschaft.getBezeichnung(),
																auspraegung.getValue(eigenschaft.getTyp()));
														auspraegung.setValue(eigenschaft.getTyp(), newVal);
														ClientsideSettings.getKontaktSharingAdministration()
																.update(auspraegung, new ReloadPage());
													}
											...
											if (currentAuspraegung.size() == 0 || eigenschaft.isMehrfach()) {
												...
												createButton.addClickHandler(new ClickHandler() {
													...
													@Override
													public void onClick(ClickEvent event) {
														String value = Window.prompt("Bitte geben Sie einen Wert fuer "
																+ eigenschaft.getBezeichnung() + " ein", "");
														...
														ClientsideSettings.getKontaktSharingAdministration()
																.createEigenschaftauspraegung(auspraegung,
																		new ReloadPageWithResponse());
													}
												});
