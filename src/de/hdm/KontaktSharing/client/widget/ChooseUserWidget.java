...
public class ChooseUserWidget extends VerticalPanel {
		...
		public ChooseUserWidget(ChooseUserPage page, Vector<Nutzer> allNutzer) {
			...
			this.getElement().setClassName("full-width");
			box.getElement().setAttribute("placeholder", "E-Mail Adresse");
			box.getElement().setClassName("full-width name-input");
		 ...
		 * @param mail
		 */
		private void addMail(final String mail) {
			final FlexTable table = new FlexTable();
			
			if(mail == "") {
				return;
			} else if (!mailIsValid(mail)) {
				Window.alert("Keine gueltige E-Mail Adresse");
				return;
			}
			SmallButton deleteButton = new SmallButton("icons/minus-small.png");
			deleteButton.addClickHandler(new ClickHandler() {
				@Override
				public void onClick(ClickEvent event) {
					removeMail(table, mail);
				}
			});
		...
		private void addMailFromInput() {
			addMail(box.getValue());
		}
		...
		private void getNutzerSuggetions() {
			  oracle.clear();
			  oracle.addAll(getSuggestions());
		}
		
		private void addOracleMail(String mail) {
			ignoredMails.remove(mail);
			getNutzerSuggetions();
		}
		
		private void removeOracleMail(String mail) {
			ignoredMails.add(mail);
			getNutzerSuggetions();
		}
		
		private List<String> getSuggestions() {
			List<String> l = new ArrayList<String>();
			for(Nutzer nutzer: allNutzer) {
				String email = nutzer.getEmail();
				boolean ignored = false;
				for(String ignoredMail: ignoredMails) {
					if(ignoredMail == email) {
						ignored = true;
					}
				}
				if(!ignored) {
